<template>
  <div>
    <tdd-line-chart
      id="tdd-member-follower-history"
      :data="followerRecords"
      :option="option"
    />
    
<!--    <div id="tdd-member-follower-history-line-chart-toolbar" style="overflow: hidden">-->
...
<!--          </p>-->
<!--          <p>数据共计：{{ this.followerRecords.length }}条，当前展示：{{ this.filteredData.length }}条。</p>-->
...
<!--      </template>-->
<!--    </div>-->
<!--    <div id="tdd-member-follower-history-line-chart"></div>-->
  </div>
</template>
...
export default {
  name: 'TddMemberFollowerHistoryLineChart',
  props: {
    followerRecords: {
      type: Array,
      required: true
  ...
  //     filteredRecords = [
  //       ...filteredRecords,
  //       ...this.followerRecords.filter(v => v.added >= this.startTs && v.added <= this.endTs)
  //     ]
  //
  ...
  //       filteredData[0].follower_speed = 0; // set first speed to 0
  //       for (let i = 1; i < filteredData.length; i++) {
  //         const view_diff = filteredData[i].follower - filteredData[i-1].follower;
  ...
  //           filteredData[i].follower_speed = filteredData[i-1].follower_speed;
  //         } else {
  //           // speed per hour
  //           filteredData[i].follower_speed = parseFloat((view_diff / added_diff * 24 * 60 * 60).toFixed(2));
  //         }
  //       }
  //     }
  //
  ...
  //       container: 'tdd-member-follower-history-line-chart',
  //       autoFit: true,
  ...
  //   if (this.followerRecords.length > 200) {
  //     defaultAddedRangeStartIndex = this.followerRecords.length - 200;
  //   }
  //   this.addedRangeValue = [
  //     moment(this.followerRecords[defaultAddedRangeStartIndex].added * 1000),
  //     moment()
  //   ];
  //   this.draw();
  // }
