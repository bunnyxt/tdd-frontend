...
export default {
  ...
  methods: {
    getSeriesList: function() {
      ...
      let startWeekTs = startTs - (startTs % (60 * 60 * 24 * 7)) // modify week to Monday
      ...
      while (p2 < records.length) {
        ...
        while (p2 < records.length) {
          ...
          let w1 = (new Date(ts1 * 1000).getDay() + 6) % 7
          let h1 = parseInt(new Date(ts1 * 1000).getHours() / 2)

          let ts2 = records[p2].added
          let v2 = records[p2].view
          let w2 = (new Date(ts2 * 1000).getDay() + 6) % 7
          let h2 = parseInt(new Date(ts2 * 1000).getHours() / 2)
